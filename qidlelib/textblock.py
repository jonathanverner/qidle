...
class TextBlock(object):
    ...
    def __init__(self, data, create_new = False, typ = None):
        if isinstance(data,QTextBlock):
            ...
            raise Exception("Invalid data to initialize block (must be a QTextBlock or QTextCursor), is "+str(type(data))+" instead.")
    ...
    def __str__(self):
        ...
        ret += "Start Pos         :" + str(self.startCursor().position()) + "\n"
        ret += "End Pos           :" + str(self.endCursor().position()) + "\n"
        ret += "Content           :" + self.content() + "\n"
        ret += "full content      :" + self.content(include_decoration = True)+"\n"
        ret += "Active area start :" + str(self.startCursor(in_active_area=True).position())+"\n"
    ...
    def leadingCursor(self):
        c = self.startCursor()
        c.movePosition(QTextCursor.Right, QTextCursor.MoveAnchor, self.indentLevel() * len(self.indent_string))
        return c
    ...
    def startCursor(self, in_active_area = True):
        c = QTextCursor(self.block)
        if in_active_area:
            c.movePosition( QTextCursor.Right, QTextCursor.MoveAnchor, self.active_area_start )
        return c
    ...
    def containsCursor(self, cursor, in_active_area = True):
        ...
        if in_active_area == 'strict':
            ...
            ret = self.startCursor(in_active_area) <= cursor <= self.endCursor()
        #debug("containsCursor: ", cursor.position(), " in [", self.startCursor(in_active_area).position(), ",",self.endCursor().position(),"], result:", ret)
        return ret
